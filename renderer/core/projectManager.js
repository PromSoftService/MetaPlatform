...
export function createProjectManager({ logger, fileSystem, moduleRegistry, onProjectLoaded, onProjectClosed }) {
  ...
  async function promoteModuleDirectory(targetRoot, moduleFolder) {
    const moduleDir = toFileSystemPath(joinProjectPath(targetRoot, moduleFolder));
    const stagingDir = toFileSystemPath(buildModuleStagingDir(targetRoot, moduleFolder));
    const backupDir = toFileSystemPath(buildModuleBackupDir(targetRoot, moduleFolder));
  ...
  async function promoteProjectFile(targetRoot, projectFileName) {
    const stagingProjectFilePath = toFileSystemPath(joinProjectPath(buildSaveStagingRoot(targetRoot), projectFileName));
    const finalProjectFilePath = toFileSystemPath(buildProjectFilePath(targetRoot, projectFileName));
    const backupProjectFilePath = toFileSystemPath(buildProjectFileBackupPath(targetRoot, projectFileName));
  ...
  async function cleanupSaveBackups(projectOwnedPaths) {
    ...
    await fileSystem.deleteFile(toFileSystemPath(buildProjectFileBackupPath(projectOwnedPaths.rootPath, projectOwnedPaths.projectFileName)));
  }
  ...
  async function saveProjectToRoot(targetProjectFilePath, openDocuments = []) {
    ...
    for (const record of docs) {
      ...
      const finalPath = toFileSystemPath(joinProjectPath(targetRoot, moduleFolder, fileName));
      const stagingPath = toFileSystemPath(joinProjectPath(stagingRoot, moduleFolder, fileName));
    ...
    // stage project.yaml
    const stagingProjectFilePath = toFileSystemPath(joinProjectPath(stagingRoot, targetProjectFileName));
    ...
    try {
      ...
      // Defensive guard: even if rollback implementation changes in future
      // and stops throwing, save/save-as must never continue via success path
      // after commit/promote failure.
      throw error;
    }
